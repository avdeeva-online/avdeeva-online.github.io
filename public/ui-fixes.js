(()=>{
  ...
  function enhance(bot){
    ...
    if(png){png.href=bot.downloadPng||String(bot.download||'').replace(/\/card(?:$|\?)/,'/card.png$1');png.classList.toggle('disabled',!png.getAttribute('href'))}
  ...
  if(typeof original==='function'){
    window.openModal=function(bot,...args){const out=original.call(this,bot,...args);enhance(bot);return out};
  }

  window.addEventListener('archive:open-character',e=>{
    const bot=e.detail?.bot;if(!bot)return;
    if(typeof window.openModal==='function')window.openModal(bot);else enhance(bot);
  });

  const modal=$('#modal');
  if(modal){
    const mo=new MutationObserver(()=>{if(!modal.hidden)enhance(findCurrentBot())});
