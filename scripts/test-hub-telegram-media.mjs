...
assert.equal((await (await suggest(0)).json()).duplicate,true,'same link is reported as already queued');
...
console.log('ARCHIVE.EXE HUB media behavior OK · suggestions + post text cleanup + Telegram CDN + manual covers stored as files, editor re-save keeps embedded cover, expired covers repaired from source post');
