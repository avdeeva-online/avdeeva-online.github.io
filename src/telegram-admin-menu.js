...
export async function tryHandleAdminMenuRequest(request,env){
  ...
  try{
    if(update.callback_query){
      ...
      await edit(token,q.message?.chat?.id,q.message?.message_id,menuText,adminMenu());
      return okResponse();
    }
    ...
    await send(token,message?.chat?.id,menuText,adminMenu());
    return okResponse();
  }catch(e){
