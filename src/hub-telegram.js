...
export async function analyzeTelegramPost(request){
  ...
  const urls=[`https://t.me/${parsed.channel}/${parsed.postId}?embed=1&mode=tme`,`https://t.me/s/${parsed.channel}/${parsed.postId}`];
  let html='',sourceUrl='';
  ...
  const textHtml=firstMatch(block,/<div class="tgme_widget_message_text[^>]*>([\s\S]*?)<\/div>/i)
    || firstMatch(html,/<meta property="og:description" content="([\s\S]*?)"\s*\/?>/i);
  ...
  const authorHtml=firstMatch(block,/<div class="tgme_widget_message_author[^>]*>[\s\S]*?<span[^>]*>([\s\S]*?)<\/span>/i)
    || firstMatch(html,/<meta property="og:title" content="([\s\S]*?)"\s*\/?>/i);
