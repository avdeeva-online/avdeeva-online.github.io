...
const isEmbeddedImageUrl=url=>/^data:image\/[a-z0-9.+-]+;base64,/i.test(clean(url));
const embeddedMediaPath=(resourceId,index)=>`/api/hub-resources/${encodeURIComponent(resourceId)}/media/${index}`;
const publicMediaUrl=(url,resourceId,index)=>isEmbeddedImageUrl(url)?embeddedMediaPath(resourceId,index):clean(url);
function publicMedia(rawMedia,resourceId){return rawMedia.map((item,index)=>{const original=mediaUrl(item),url=publicMediaUrl(original,resourceId,index);if(!url||url===original)return item;return item?.url!=null?{...item,url}:{...item,src:url}})}
...
function resourceObject(r,allFiles){const sourceFiles=allFiles.filter(f=>f.source),files=allFiles.filter(f=>!f.source),rawMedia=arr(parseJson(r.media,[])).filter(Boolean),safeMedia=publicMedia(rawMedia,r.id),normalFiles=files.filter(f=>!f.extra),normalImages=normalFiles.filter(f=>isImage(f.name,f.mime)),explicitExtras=files.filter(f=>f.extra&&isImage(f.name,f.mime)),sourceImages=sourceFiles.filter(f=>isImage(f.name,f.mime));const selectedIndex=rawMedia.findIndex(m=>m?.cover&&mediaUrl(m)),firstIndex=rawMedia.findIndex(m=>mediaUrl(m)),coverIndex=selectedIndex>=0?selectedIndex:firstIndex,storedCover=normalImages.find(f=>f.primary)||normalImages[0]||null,coverUrl=coverIndex>=0?mediaUrl(safeMedia[coverIndex]):storedCover?.view_url||'',coverFallbackUrls=[...new Set([...safeMedia.map(mediaUrl),storedCover?.view_url,explicitExtras[0]?.view_url,sourceImages.find(f=>/cover/i.test(f.name))?.view_url,sourceImages[0]?.view_url].map(clean).filter(x=>x&&x!==coverUrl))];let media=safeMedia.map(m=>({...m,cover:mediaUrl(m)===coverUrl}));if(coverUrl&&!media.some(m=>mediaUrl(m)===coverUrl))media=[{url:coverUrl,cover:true,source:'stored-file'},...media.map(m=>({...m,cover:false}))];if(media.length&&!media.some(m=>m.cover))media[0].cover=true;return{id:r.id,source_url:r.source_url,type:r.type,title:r.title,creator:{name:r.creator_name,link:r.creator_link},description_short:r.description_short,description_full:r.description_full,additional_info:r.additional_info||'',models:parseJson(r.models),settings:normalizeSettings(parseJson(r.settings)),tags:arr(parseJson(r.tags)).filter(x=>clean(x).toLowerCase()!=='magic'),media,cover_url:coverUrl,cover_fallback_urls:coverFallbackUrls,primary_file_id:normalFiles.find(f=>f.primary)?.id||null,file_count:normalFiles.length,extra_count:explicitExtras.length,extra_images:explicitExtras,files,source_files:sourceFiles,updated_at:r.updated_at}}
...
export async function listHubResourcesSummaryPublic(env){
  try{
    ...
    const resources=rows.map(r=>{const rawMedia=arr(parseJson(r.media,[])).filter(Boolean),safeMedia=publicMedia(rawMedia,r.id),selectedIndex=rawMedia.findIndex(m=>m?.cover&&mediaUrl(m)),firstIndex=rawMedia.findIndex(m=>mediaUrl(m)),coverIndex=selectedIndex>=0?selectedIndex:firstIndex,fallback=info.get(r.id)||{files:0,cover:'',extraCover:'',sourceCover:''},coverUrl=coverIndex>=0?mediaUrl(safeMedia[coverIndex]):fallback.cover||'',coverFallbackUrls=[...new Set([...safeMedia.map(mediaUrl),fallback.cover,fallback.extraCover,fallback.sourceCover].map(clean).filter(x=>x&&x!==coverUrl))];return{id:r.id,type:r.type,title:r.title,creator:{name:r.creator_name},description_short:r.description_short,models:parseJson(r.models),settings:normalizeSettings(parseJson(r.settings)),tags:arr(parseJson(r.tags)).filter(x=>clean(x).toLowerCase()!=='magic'),cover_url:coverUrl,cover_fallback_urls:coverFallbackUrls,file_count:fallback.files,updated_at:r.updated_at}});
...
export async function getHubEmbeddedMediaPublic(env,resourceId,index){
  try{
    const row=await env.DB.prepare(`SELECT media FROM hub_resources WHERE id=? AND status='published' LIMIT 1`).bind(resourceId).first();if(!row)return json({ok:false,error:'RESOURCE_NOT_FOUND'},404);
    const media=arr(parseJson(row.media,[])),raw=mediaUrl(media[Number(index)]);if(!isEmbeddedImageUrl(raw))return json({ok:false,error:'EMBEDDED_MEDIA_NOT_FOUND'},404);
    const match=raw.match(/^data:(image\/[a-z0-9.+-]+);base64,([a-z0-9+/=\s]+)$/i);if(!match)return json({ok:false,error:'EMBEDDED_MEDIA_INVALID'},422);
    let bytes;try{const binary=atob(match[2].replace(/\s+/g,''));bytes=Uint8Array.from(binary,c=>c.charCodeAt(0))}catch{return json({ok:false,error:'EMBEDDED_MEDIA_INVALID'},422)}
    return new Response(bytes,{status:200,headers:{'content-type':match[1].toLowerCase(),'content-length':String(bytes.byteLength),'cache-control':'public, max-age=86400, stale-while-revalidate=604800','x-content-type-options':'nosniff'}});
  }catch(e){console.error('getHubEmbeddedMediaPublic failed',e);return json({ok:false,error:'HUB_EMBEDDED_MEDIA_READ_FAILED',detail:String(e?.message||e)},500)}
}
