(()=>{
  ...
  function styles(){
    ...
    const s=document.createElement('style');s.id='archiveImportStyles';s.textContent=`
      ...
      .archive-import-backdrop{position:absolute;inset:0;background:rgba(4,7,4,.82);backdrop-filter:blur(5px)}
      .archive-import-card{position:relative;width:min(650px,calc(100vw - 32px));background:linear-gradient(145deg,#121712,#0d110d);border:1px solid #46513f;border-radius:10px;box-shadow:0 28px 90px rgba(0,0,0,.72);padding:22px 22px 18px;color:#d8deca;font-family:Consolas,monospace}
      .archive-import-card:before{content:"";position:absolute;inset:5px;border:1px solid rgba(122,141,107,.08);border-radius:7px;pointer-events:none}
      .archive-import-card h2{margin:0 36px 4px 0;font-size:18px;letter-spacing:.05em}.archive-import-card>p{margin:0 0 16px;color:#7e8a76;font-size:10px;letter-spacing:.04em}
      .archive-import-close{position:absolute;right:13px;top:10px;width:30px;height:30px;border:1px solid transparent;border-radius:50%;background:none;color:#87937f;font-size:22px;cursor:pointer}.archive-import-close:hover{border-color:#46513f;color:#dce5d2}
      ...
      .archive-import-go{border:1px solid #647455;border-radius:6px;background:#27331f;color:#e3ead7;padding:0 17px;font:11px Consolas,monospace;cursor:pointer}.archive-import-go:disabled{opacity:.45;cursor:wait}
      .archive-import-state{margin:11px 1px 0;color:#9eae91;font-size:10px;min-height:14px}.archive-import-state.error{color:#d49784}.archive-import-state.ok{color:#b9cf8f}
      @media(max-width:620px){.archive-import-row{grid-template-columns:1fr}.archive-import-go{padding:11px}}
    `;document.head.appendChild(s);
  }

  function resetImport(){
    const input=$('#archiveImportUrl'),state=$('#archiveImportState'),go=$('#archiveImportGo');
    if(input)input.value='';
    if(state){state.textContent='READY.';state.className='archive-import-state'}
  ...
  function buildModal(){
    ...
    const box=document.createElement('div');box.id='archiveImportModal';box.className='archive-import-modal';box.hidden=true;box.innerHTML=`
      ...
      <section class="archive-import-card">
        <button class="archive-import-close" data-import-close aria-label="Close">×</button>
    ...
    box.querySelectorAll('[data-import-close]').forEach(x=>x.onclick=closeImport);
  ...
  function bindTrigger(){
    ...
    if(existing){existing.classList.add('archive-import-trigger');existing.onclick=openImport;return}
  }
  ...
  async function runImport(){
    ...
    go.disabled=true;go.textContent='IMPORTING…';setState('CHECKING ARCHIVE → DATACAT…');
    try{
      ...
      setState('IMPORTED. OPENING RECORD…','ok');
      setTimeout(()=>{
        closeImport();
        if(typeof window.openModal==='function')window.openModal(bot);
        else window.dispatchEvent(new CustomEvent('archive:open-character',{detail:{bot}}));
      },180);
    }catch(e){setState(String(e.message||e),'error');go.disabled=false;go.textContent='IMPORT'}
  }
  ...
  const start=()=>{baseBots=Array.isArray(window.BOTS)?[...window.BOTS]:baseBots;bindTrigger();buildModal();loadAddon('lorebooks.js','archive-lorebooks');loadAddon('ui-fixes.js','archive-ui-fixes');loadLive()};
