...
const isImageMeta=(name,mime)=>/^image\//i.test(clean(mime))||/\.(png|jpe?g|webp|gif)$/i.test(clean(name));
const isExtraMeta=(name,mime)=>clean(name).startsWith(EXTRA_PREFIX)||isImageMeta(name,mime);
let ready=null;
...
export async function publishHubResource(request,env){
  try{
    ...
    if(markedName)await env.DB.prepare("UPDATE hub_resource_files SET is_primary=0 WHERE resource_id=? AND name NOT LIKE '__extra__%'").bind(id).run();

    const storeLocal=async(f,isExtra=false)=>{
      let name=clean(f.name)||`file-${crypto.randomUUID()}`;
      if(isExtra&&!name.startsWith(EXTRA_PREFIX))name=EXTRA_PREFIX+name;
      const buf=await f.arrayBuffer(),makePrimary=!isExtra&&Boolean(markedName&&name===markedName);
      if(old?.id){
    ...
    const hasPrimary=await env.DB.prepare("SELECT id FROM hub_resource_files WHERE resource_id=? AND is_primary=1 AND name NOT LIKE '__extra__%' LIMIT 1").bind(id).first();
    if(!hasPrimary){const first=await env.DB.prepare("SELECT id FROM hub_resource_files WHERE resource_id=? AND name NOT LIKE '__extra__%' AND mime NOT LIKE 'image/%' ORDER BY created_at ASC LIMIT 1").bind(id).first();if(first?.id)await env.DB.prepare('UPDATE hub_resource_files SET is_primary=1 WHERE id=?').bind(first.id).run()}
...
export async function deleteHubResourceFile(env,resourceId,fileId){await ensureSchema(env);const row=await env.DB.prepare('SELECT id,is_primary,name,mime FROM hub_resource_files WHERE id=? AND resource_id=? LIMIT 1').bind(fileId,resourceId).first();if(!row)return json({ok:false,error:'FILE_NOT_FOUND'},404);await env.DB.prepare('DELETE FROM hub_resource_files WHERE id=? AND resource_id=?').bind(fileId,resourceId).run();if(row.is_primary&&!isExtraMeta(row.name,row.mime)){const next=await env.DB.prepare("SELECT id FROM hub_resource_files WHERE resource_id=? AND name NOT LIKE '__extra__%' AND mime NOT LIKE 'image/%' ORDER BY created_at ASC LIMIT 1").bind(resourceId).first();if(next?.id)await env.DB.prepare('UPDATE hub_resource_files SET is_primary=1 WHERE id=?').bind(next.id).run()}return json({ok:true})}
export async function setHubResourcePrimary(env,resourceId,fileId){await ensureSchema(env);const row=await env.DB.prepare('SELECT id,name,mime FROM hub_resource_files WHERE id=? AND resource_id=? LIMIT 1').bind(fileId,resourceId).first();if(!row)return json({ok:false,error:'FILE_NOT_FOUND'},404);if(isExtraMeta(row.name,row.mime))return json({ok:false,error:'EXTRA_IMAGE_CANNOT_BE_PRIMARY'},400);await env.DB.prepare('UPDATE hub_resource_files SET is_primary=0 WHERE resource_id=?').bind(resourceId).run();await env.DB.prepare('UPDATE hub_resource_files SET is_primary=1 WHERE id=? AND resource_id=?').bind(fileId,resourceId).run();return json({ok:true})}
...
export async function listHubResources(env){
  await ensureSchema(env);
  const res=await env.DB.prepare(`SELECT r.*, (SELECT id FROM hub_resource_files f WHERE f.resource_id=r.id AND f.name NOT LIKE '__extra__%' AND f.mime NOT LIKE 'image/%' ORDER BY is_primary DESC,created_at ASC LIMIT 1) primary_file_id,(SELECT COUNT(*) FROM hub_resource_files f WHERE f.resource_id=r.id AND f.name NOT LIKE '__extra__%' AND f.mime NOT LIKE 'image/%') file_count FROM hub_resources r WHERE status='published' ORDER BY updated_at DESC`).all();
  const fileRes=await env.DB.prepare(`SELECT id,resource_id,name,mime,size,is_primary,external_url FROM hub_resource_files ORDER BY is_primary DESC,created_at ASC`).all(),filesByResource=new Map();
  for(const f of fileRes.results||[]){if(!filesByResource.has(f.resource_id))filesByResource.set(f.resource_id,[]);filesByResource.get(f.resource_id).push({id:f.id,name:f.name,mime:f.mime,size:Number(f.size||0),primary:Boolean(f.is_primary),extra:isExtraMeta(f.name,f.mime),external_url:f.external_url||'',download_url:`/api/hub-resources/${encodeURIComponent(f.resource_id)}/files/${encodeURIComponent(f.id)}`})}
