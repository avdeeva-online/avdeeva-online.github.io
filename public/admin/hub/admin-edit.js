(()=>{
  ...
  function stripSchool(){const b=document.querySelector('#settingChoices .choice[data-v="school-university"]');if(!b)return;if(b.classList.contains('active'))b.click();b.remove()}
  stripSchool();
  new MutationObserver(stripSchool).observe(document.body,{subtree:true,childList:true});
  ...
  function choose(boxSelector,values,multi=true){const box=$(boxSelector);if(!box)return;const wanted=new Set(Array.isArray(values)?values:[values]);box.querySelectorAll('.choice').forEach(b=>{const should=wanted.has(b.dataset.v),active=b.classList.contains('active');if(should!==active)b.click()})}
  function selected(boxSelector){return[...document.querySelectorAll(`${boxSelector} .choice.active`)].map(b=>b.dataset.v)}
  function snapshotManual(){return{title:$('#title')?.value||'',creator:$('#creator')?.value||'',creatorLink:$('#creatorLink')?.value||'',short:$('#shortDesc')?.value||'',full:$('#fullDesc')?.value||'',extra:$('#extraTags')?.value||'',additional:$('#additionalInfo')?.value||'',type:selected('#typeChoices')[0]||'other',models:selected('#modelChoices'),settings:selected('#settingChoices').filter(x=>allowedSettings.has(x))}}
