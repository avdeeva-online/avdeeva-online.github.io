...
for(const marker of ["from './janny-card.js'",'fetchJannySource(uuid)','definitionFromEmbeddedCard(janny.card)','mergeDefinition(definition,recovered)','definition_available:definitionAvailable','definition_source:recovery||"datacat"','export_quality:definitionAvailable?"complete":"partial"','SOURCE_DEFINITION_UNAVAILABLE','source_diagnostics:definitionAvailable?undefined:diagnostics','schemaPaths:view.state===\'FOUND\'?schemaPaths(view.data):[]','JANNY_PNG_NO_EMBEDDED_CARD'])fail(workerBase.includes(marker),`src/worker.js: Janny recovery/truthful partial diagnostics missing ${marker}`);
