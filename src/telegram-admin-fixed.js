...
function addLogicalSource(a,meta,message){a.sourcePosts=Array.isArray(a.sourcePosts)?a.sourcePosts:[];a.diagnostics=a.diagnostics||{};a.diagnostics.mediaGroups=a.diagnostics.mediaGroups||{};const gid=clean(message?.media_group_id);if(gid){if(a.diagnostics.mediaGroups[gid])return;a.diagnostics.mediaGroups[gid]=meta?.url||''}const url=meta?.url||'';if(url&&!a.sourcePosts.some(x=>(typeof x==='string'?x:x?.url)===url))a.sourcePosts.push({url})}

async function attachFileOnly(env,active,message){const {row}=active,a=row.analysis||{};a.source=a.source||{};a.draft=a.draft||{};const meta=forwardedMeta(message),source=meta?.url||'';addLogicalSource(a,meta,message);a.files=uniqBy([...(a.files||[]),...directFiles(message,source)],x=>x.telegram_file_unique_id||x.telegram_file_id||x.url||`${x.name}:${x.size}`);a.diagnostics={...(a.diagnostics||{}),attachmentOnly:true,directFiles:a.files.filter(x=>x.telegram_file_id).length};await env.DB.prepare("UPDATE telegram_admin_drafts SET payload=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(JSON.stringify(a),row.id).run();row.analysis=a;return row}
async function attachTextOnly(env,active,message){const {row}=active,a=row.analysis||{};a.source=a.source||{};a.draft=a.draft||{};const text=clean(message?.text||message?.caption);if(!text)return row;const shouldAdopt=weakContent(a)||text.length>clean(a.draft.descriptionFull||a.source.rawText).length*1.8;if(shouldAdopt){a.source.rawText=text;a.draft.descriptionFull=text;a.draft.descriptionShort=short(text,520);const t=titleFromText(text);if(t)a.draft.title=t;a.draft.type=inferType(text,a.draft.type)}else{a.diagnostics=a.diagnostics||{};a.diagnostics.extraTexts=uniqBy([...(a.diagnostics.extraTexts||[]),text],x=>x)}await env.DB.prepare("UPDATE telegram_admin_drafts SET payload=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(JSON.stringify(a),row.id).run();row.analysis=a;return row}
...
export async function handleAdminTelegramFixed(request,env){
  ...
  try{
    ...
    if(fileOnly){const row=await attachFileOnly(env,active,message);await send(token,message.chat.id,`<b>FILES ADDED TO ACTIVE RESOURCE ✓</b>\n\n${draftText(row)}`,buttons(row.id,row.source_url));return new Response(JSON.stringify({ok:true}),{headers:{'content-type':'application/json'}})}
    const meta=forwardedMeta(message);
    if(!meta&&text&&!(message?.photo?.length)){const row=await attachTextOnly(env,active,message);await send(token,message.chat.id,`<b>DESCRIPTION ADDED TO ACTIVE RESOURCE ✓</b>\n\n${draftText(row)}`,buttons(row.id,row.source_url));return new Response(JSON.stringify({ok:true}),{headers:{'content-type':'application/json'}})}
