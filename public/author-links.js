(()=>{
  ...
  function patchBots(list){
    ...
    for(const b of list){if(b)b.authorUrl=verifiedUrl(b.author,b.authorUrl)}
  }

  function currentAuthor(){return clean(document.getElementById('modalAuthor')?.textContent).replace(/^@/,'')}
  ...
  function patchModal(){
    ...
    if(u){
      a.href=u;a.textContent=`@${author} ↗`;a.classList.remove('disabled');a.removeAttribute('aria-disabled');a.target='_blank';a.rel='noopener noreferrer';
      ...
      a.removeAttribute('href');a.textContent='AUTHOR LINK — N/A';a.classList.add('disabled');a.setAttribute('aria-disabled','true');
  ...
  document.addEventListener('click',e=>{
    ...
    if(!badge)return;
    patchModal();const u=badge.dataset.authorUrl;if(!u)return;
    e.preventDefault();e.stopPropagation();window.open(u,'_blank','noopener,noreferrer');
  },true);

  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',patchModal,{once:true});else patchModal();
})();