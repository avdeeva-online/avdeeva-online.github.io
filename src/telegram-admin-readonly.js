...
async function showDrafts(token,chatId,messageId,env){
  await ensureDraftSchema(env);
  const rows=(await env.DB.prepare("SELECT id,source_url,payload,status,updated_at FROM telegram_admin_drafts WHERE status='review' ORDER BY updated_at DESC LIMIT 8").all()).results||[];
  ...
  for(const r of rows){
    let title='UNTITLED';
    try{title=JSON.parse(r.payload||'{}')?.draft?.title||title}catch{}
    keys.push([button(short(title,36),'draft:view:'+r.id)]);
  }
...
export async function tryHandleAdminReadonlyRequest(request,env){
  ...
  if(!q||!['adm:hub','adm:authors','adm:issues','adm:drafts'].includes(data))return null;
  ...
  try{
    ...
    else await showDrafts(token,q.message?.chat?.id,q.message?.message_id,env);
  }catch(e){
