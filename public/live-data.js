(()=>{
  ...
  function preview(bot){
    ...
    const uuid=bot.janitorUuid||uuidFrom(bot.url);const actions=$('#archiveImportActions');actions.innerHTML=`<a class="primary" href="/api/characters/${esc(uuid)}/card.png" target="_blank">PNG CARD ↓</a><a href="/api/characters/${esc(uuid)}/card">JSON CARD ↓</a>${bot.lorebook?`<a href="${esc(bot.lorebook)}">LOREBOOK ↓</a>`:''}<a href="${esc(bot.url)}" target="_blank" rel="noopener">JANITOR ↗</a><button id="archiveViewCatalog">VIEW IN CATALOG</button>`;
  ...
  document.addEventListener('DOMContentLoaded',()=>{baseBots=Array.isArray(window.BOTS)?[...window.BOTS]:baseBots;addTrigger();buildModal();loadLive()});
})();
