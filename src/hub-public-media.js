...
export async function listHubResourcesSummaryPublic(env){
  try{
    ...
    const info=new Map();for(const f of fileRows){if(isSource(f.name))continue;const x=info.get(f.resource_id)||{files:0,cover:''};if(!isExtra(f.name)){x.files++;if(!x.cover&&isImage(f.name,f.mime)){const urls=fileUrls(f.resource_id,f.id);x.cover=clean(f.external_url)||urls.view_url}}info.set(f.resource_id,x)}
    const resources=rows.map(r=>{const rawMedia=arr(parseJson(r.media,[])).filter(Boolean),selected=rawMedia.find(m=>m?.cover&&mediaUrl(m))||rawMedia.find(m=>mediaUrl(m)),fallback=info.get(r.id)||{files:0,cover:''};return{id:r.id,type:r.type,title:r.title,creator:{name:r.creator_name},description_short:r.description_short,models:parseJson(r.models),settings:normalizeSettings(parseJson(r.settings)),tags:arr(parseJson(r.tags)).filter(x=>clean(x).toLowerCase()!=='magic'),cover_url:mediaUrl(selected)||fallback.cover||'',file_count:fallback.files,updated_at:r.updated_at}});
