...
for(const marker of ['const visibleBotHashtags = bot =>','const hashtags = visibleBotHashtags(b)','hashtags.slice(0,6)','function fitCardChips(','const tags=visibleBotTags(b),hashtags=visibleBotHashtags(b)'])fail(publicApp.includes(marker),`public/app.js: visible hashtag dedupe missing ${marker}`);
...
for(const file of ['app.js','catalog-api.js'])fail(new RegExp(`${file.replace('.','\\.')}\\?v=[\\w-]+`).test(charactersHtml),`public/characters.html: ${file} cache-bust version missing`);
...
for(const marker of ['const isPovTag = value =>','if(isPovTag(tag))return','const visibleBotTags = bot =>','const tags = visibleBotTags(b)','const tags=visibleBotTags(b),hashtags=visibleBotHashtags(b)','<span class="cm-fact cm-fact-pov">${esc(povLabel(botPov(b)))}</span>','card-status-icon card-pov-icon','povSymbol(pov)'])fail(publicApp.includes(marker),`public/app.js: separated/canonical POV-tag contract missing ${marker}`);
fail(publicApp.includes('function renderModalHead()'),'public/app.js: record card head renderer missing');
