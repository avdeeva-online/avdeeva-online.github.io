...
export async function listHubResourcesPublic(env){
  try{
    ...
    const resources=(res.results||[]).map(r=>{
      const allFiles=byResource.get(r.id)||[],sourceFiles=allFiles.filter(f=>f.source),files=allFiles.filter(f=>!f.source),rawMedia=arr(parseJson(r.media,[])).filter(Boolean),normalFiles=files.filter(f=>!f.extra),normalImages=normalFiles.filter(f=>isImage(f.name,f.mime)),explicitExtras=files.filter(f=>f.extra&&isImage(f.name,f.mime));
      ...
      const coverUrl=mediaUrl(selectedMedia)||mediaUrl(firstMedia)||storedCover?.view_url||'';
...
export async function hubMediaAudit(env){
  try{
    ...
    for(const r of data.resources)for(const f of [...arr(r.files),...arr(r.source_files)]){const k=f.external_url?'remote':(f.storage==='r2'?'r2':'d1');storage[k]=(storage[k]||0)+1}
