(()=>{
  ...
  box.innerHTML=`<div class="cover-preview">NO MANUAL COVER</div><div class="cover-tools"><div class="cover-title">MANUAL COVER IMAGE</div><div class="cover-copy">Use this only when Telegram did not return the correct image. The resource editor will use it as the public card/modal cover.</div><div class="cover-actions"><input id="manualCoverInput" type="file" accept="image/png,image/jpeg,image/webp" hidden><button class="cover-btn primary" id="manualCoverPick" type="button">CHOOSE COVER</button><button class="cover-btn remove" id="manualCoverRemove" type="button">REMOVE</button></div><div class="cover-status">JPG / PNG / WEBP · automatically optimized</div></div>`;
  ...
  window.archiveHubSetManualCover=url=>{manualCover=String(url||'');setPreview(manualCover);status.textContent=manualCover?'MANUAL COVER READY':'NO MANUAL COVER'};
  box.querySelector('#manualCoverPick').onclick=()=>input.click();
  box.querySelector('#manualCoverRemove').onclick=()=>{manualCover='';input.value='';setPreview('');status.textContent='MANUAL COVER REMOVED';window.dispatchEvent(new CustomEvent('archive:hub-cover-change'))};
  input.onchange=async()=>{const file=input.files?.[0];if(!file)return;status.textContent='OPTIMIZING COVER...';try{manualCover=await optimize(file);setPreview(manualCover);status.textContent=`READY · ${Math.round(manualCover.length*0.75/1024)} KB APPROX.`;window.dispatchEvent(new CustomEvent('archive:hub-cover-change'))}catch(e){manualCover='';setPreview('');status.textContent='FAILED: '+e.message}};
})();
