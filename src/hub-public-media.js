...
export async function listHubResourcesPublic(env){
  try{
    ...
    for(const f of fileRes.results||[]){const mime=inferMime(f.name,f.mime),urls=fileUrls(f.resource_id,f.id),extra=isExtra(f.name),obj={id:f.id,name:f.name,mime,size:Number(f.size||0),primary:Boolean(f.is_primary),extra,explicit_extra:extra,external_url:clean(f.external_url),attachment_url:urls.attachment_url,view_url:clean(f.external_url)||urls.view_url,download_url:extra&&isImage(f.name,mime)?(clean(f.external_url)||urls.view_url):urls.attachment_url,created_at:f.created_at||''};if(!byResource.has(f.resource_id))byResource.set(f.resource_id,[]);byResource.get(f.resource_id).push(obj)}
    const resources=(res.results||[]).map(r=>{
      ...
      let media=rawMedia.map(m=>({...m,cover:mediaUrl(m)===coverUrl}));if(coverUrl&&!media.some(m=>mediaUrl(m)===coverUrl))media=[{url:coverUrl,cover:true,source:'stored-file'},...media.map(m=>({...m,cover:false}))];if(media.length&&!media.some(m=>m.cover))media[0].cover=true;
    ...
    return json({ok:true,resources,count:resources.length,media_model:4});
...
export async function downloadHubFilePublic(request,env,resourceId,fileId){
  try{
    ...
    if(!row)return json({ok:false,error:'FILE_NOT_FOUND'},404);if(row.external_url)return Response.redirect(row.external_url,302);
    let body;if(row.data!=null){body=toBytes(row.data);if(!body.byteLength&&Number(row.size||0)>0)return json({ok:false,error:'FILE_DATA_INVALID'},500)}else{const chunks=await env.DB.prepare('SELECT data FROM hub_resource_file_chunks WHERE file_id=? ORDER BY chunk_index ASC').bind(fileId).all();if(!(chunks.results||[]).length)return json({ok:false,error:'FILE_DATA_MISSING'},404);const parts=(chunks.results||[]).map(x=>toBytes(x.data)),total=parts.reduce((n,p)=>n+p.byteLength,0),joined=new Uint8Array(total);let offset=0;for(const p of parts){joined.set(p,offset);offset+=p.byteLength}body=joined}
    const mime=inferMime(row.name,row.mime),url=new URL(request.url),view=url.searchParams.get('view')==='1',inline=view&&isImage(row.name,mime),headers=new Headers({'content-type':mime,'cache-control':'public, max-age=86400','content-length':String(body.byteLength),'x-content-type-options':'nosniff'});headers.set('content-disposition',`${inline?'inline':'attachment'}; filename*=UTF-8''${encodeURIComponent(clean(row.name)||(inline?'image':'download'))}`);return new Response(body,{status:200,headers});
...
export async function hubMediaAudit(env){try{const response=await listHubResourcesPublic(env),data=await response.clone().json();if(!response.ok||!data.ok)return response;return json({ok:true,count:data.resources.length,resources:data.resources.map(r=>({id:r.id,title:r.title,cover:Boolean(r.cover_url),media_count:arr(r.media).length,file_count:arr(r.files).filter(f=>!f.extra).length,image_files:arr(r.files).filter(f=>isImage(f.name,f.mime)&&!f.extra).length,extras:arr(r.files).filter(f=>f.extra&&isImage(f.name,f.mime)).length}))})}catch(e){return json({ok:false,error:'HUB_MEDIA_AUDIT_FAILED',detail:String(e?.message||e)},500)}}