(()=>{
  ...
  function resetImport(){const input=$('#archiveImportUrl'),state=$('#archiveImportState'),go=$('#archiveImportGo');if(input&&!importBusy)input.value='';if(state){state.textContent=importBusy?'IMPORT IN PROGRESS…':'READY.';state.className='archive-import-state'}if(go){go.disabled=importBusy;go.textContent=importBusy?'IMPORTING…':'IMPORT'}}
  function closeImport(){const box=$('#archiveImportModal');if(box)box.hidden=true;resetImport()}
  function openImport(){buildModal();resetImport();const box=$('#archiveImportModal');if(box)box.hidden=false;if(!importBusy)setTimeout(()=>$('#archiveImportUrl')?.focus(),0)}
  ...
  function openGatewayRandom(){
    const params=new URLSearchParams(location.search);
    if(params.get('gatewayRandom')!=='1')return;
    params.delete('gatewayRandom');
    const next=`${location.pathname}${params.toString()?`?${params}`:''}${location.hash||''}`;
    history.replaceState(null,'',next);
    setTimeout(()=>$('#randomBtn')?.click(),180);
  }
  const start=()=>{
    ...
    loadLive().then(openGatewayRandom).finally(()=>loadAddon('lorebooks.js','archive-lorebooks'));
  };
